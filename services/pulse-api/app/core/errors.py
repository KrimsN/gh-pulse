...
Нужен и в самих хендлерах (`repo_card`, задача 2.4 — 404 на несуществующий репозиторий), и в
зависимостях (`app/security/api_key.py`, `app/security/rate_limit.py`), которые не имеют доступа
к `Response` эндпоинта и должны прервать обработку запроса раньше вызова хендлера — `raise ApiError`
даёт единый конверт через один обработчик исключений везде, не дублируя `JSONResponse(...)` вручную
в каждом месте.
"""
