...
flowchart LR
    ...
    subgraph Наблюдаемость
        O[OpenTelemetry → Jaeger / Prometheus → Grafana]
    end
...
| Наблюдаемость | **OpenTelemetry → Jaeger (трейсы) / Prometheus → Grafana (метрики)** | Трассировка, метрики и дашборды на одном открытом стеке, см. [ADR 0009](adr/0009-opentelemetry-tracing-jaeger.md) |
...
| Jaeger (UI / OTLP-gRPC) | `jaeger` | 16686, 4317 | 16686, 4317 |
...
даже там, где нет Redis-кэша (`app/api/http_cache.py`). `/trending` — единственный по-настоящему
...
меняться между запросами двух страниц одного клиента (`app/api/pagination.py`).
...
(`services/pulse-api/app/api/query_params.py`).
