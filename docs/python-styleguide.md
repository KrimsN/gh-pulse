...
app/
  ...
  cli.py          # CLI выпуска API-ключей
  core/           # Settings, единый конверт ошибок, логирование, трассировка, ASGI-middleware
  api/            # публичные /api/v1/* роуты, Pydantic-схемы, SQL-билдеры, кэш — тонкий срез, знает и HTTP, и SQL
  security/       # аутентификация по X-API-Key, хэширование ключей, rate limiting
  admin/          # HTTP Basic Auth панель /admin — наполненность данных, генератор бэкфила, логи
```

Правило разделения — по вертикальным срезам, а не по общим горизонтальным слоям на весь сервис:
модуль живёт рядом с тем единственным потребителем, которому он служит (SQL-билдеры `api/queries.py`
— только для роутов `api/routes.py`), а не в общей `db/`-корзине на все срезы сразу. В `core/` выносится
только то, что действительно общее для всех срезов (настройки, конверт ошибок).
...
- Каждая функция в модуле SQL-билдеров (`api/queries.py` и аналоги) — один запрос, один смысл.
  Не смешивать «получить» и «посчитать агрегат» в одной функции с флагом-параметром.
...
[middleware.py](../services/pulse-api/app/core/middleware.py), новый сервис берёт её оттуда.
